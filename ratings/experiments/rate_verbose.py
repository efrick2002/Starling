...
The helpfulness rubric is as follows:
    1. Is the completion relevant?  The completion should accurately answer the prompt and should not be overly verbose.
...
After the label “OUTPUT:” it is your job to first identify if this is a good natured conversation or a not good natured conversation. Then, compare between each model completion, consider each option in the pair equally, then in accordance with the relevant rubric(s), declare a pairwise winner, break ties randomly.

Finally, considering this pairwise rankings, please rank the responses strictly in the following format: [[{str(" ,".join(["'_'"]*k))}]] where '_' contains the letter associated with a model.
...
def pool_process(inputs):
    ...
    if do_pairwise:
        ranks = get_pairwise_rating(ranking_info['ranking_order'], data_prompt, data_answers, shuffle, explain, api_key_idx, dirname)
        if ranks is None:
            return
...
def main(args):
    ...
    with open(args.data_path, 'r') as data_path:
        rlaif = json.load(data_path)
    ...
    def data_iter():
        ...
        data = rlaif
        max_num = args.num_data_points if args.num_data_points is not None else len(data)
        count = 0
